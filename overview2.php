...
// echo '<pre>';var_dump($data_group);echo '</pre>';exit;
$arr_group = [];
...
$form_action = new moodle_url('/blocks/pintar_analytic/overview2.php', ['id' => $id]);
 
echo $OUTPUT->render_from_template('block_pintar_analytic/userranking', [
	'result' => $result,
...
# number_format(($value/$totalactivities*100),2)
	 ...
	 $data_usergroup = get_user_group_by_userid_and_courseid($user->id,$id);
      ...
      # die();
       ...
       # array_push($hasil,array("userid"=>$user->id,"user_groupid"=>groups_get_group_name($data_usergroup['usergroupid']),"nama"=>$user->firstname.' '.$user->lastname,"tactv"=>$usertotalactivities,"prouseractv"=>$prosenuseractivities));
